...
namespace WCFServer.Data.DapperEx.Commands
{
    ...
    internal class ConditionBuilder : ExpressionVisitor
    {
        ...
        private string _tableAlias = "";                          //表别名
        Dictionary<String, PropertyEx> _propertiesMap = null;
        ...
        public ConditionBuilder(string parameterPrefix, string tableAlias)
        {
            _parameterPrefix = parameterPrefix;
            _tableAlias = String.IsNullOrEmpty(tableAlias) ? "" : tableAlias + '.';
        }
        ...
        protected override Expression VisitBinary(BinaryExpression b)
        {
            ...
            if (b.NodeType == ExpressionType.And || b.NodeType == ExpressionType.AndAlso || b.NodeType == ExpressionType.OrElse)
            {
        ...
        protected override Expression VisitMemberAccess(MemberExpression m)
        {
            ...
            if (m.Expression.NodeType != ExpressionType.Parameter)
            {
                switch (m.Member.Name)
                {
                    case "Length":
                        ...
                        memberName = string.Format("LEN({1}{0})", memberName, TableAlias);
            ...
            else
            {
                ...
                this._stcConditions.Push(TableAlias + fieldName);
            }
        ...
        private string WrapMemberName(string name)
        {
            ...
            if (!string.IsNullOrEmpty(name))
            {
                if (_propertiesMap != null && _propertiesMap.ContainsKey(name))
                    return string.Format("{0}", _propertiesMap[name].DbName);
